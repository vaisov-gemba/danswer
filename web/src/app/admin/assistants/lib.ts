...
export function usePromptRefresh() {
  const refreshPrompts = useCallback(
    async (values: any, setFieldValue: (field: string, value: any) => void) => {
      try {
  ...
  const debouncedRefreshPrompts = useMemo(
    () => debounce(refreshPrompts, 1000),
    [refreshPrompts]
  );

  return { debouncedRefreshPrompts };
}
